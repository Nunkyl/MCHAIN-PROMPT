...
persistent symbols; %their values are retained in memory between calls to the function
...
mass = cell(1, length(atomicSymbols));
for i=1:length(atomicSymbols)
    mass{i} = cell2mat(values(atomicMasses, atomicSymbols{i}))';
end
